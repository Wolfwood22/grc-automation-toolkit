variable "gcp_project" {
    type        = string
    description = "GCP project ID where bucket and KMS resources will live."
}

variable "location" {
    type        = string
    description = "GCS bucket location. Multi-regions (US, EU) are valid."
    default     = "us-central1"
}

variable "kms_location" {
    type        = string
    description = "KMS keyring location. Must be a single region — multi-regions unsupported."
    default     = "us-central1"
}

variable "project_label" {
    type        = string
    description = "Short project identifier used in bucket name and labels."
    validation {
...
variable "retention_days" {
    type        = number
    description = "Object retention in days. Production requires >= 365."
...
variable "labels" {
    type        = map(string)
    description = "Optional additional labels. Compliance labels are merged on top and cannot be overridden."
    default     = {}
}